...
class SessionManager:
    """
    Orchestrates the lifecycle of an experimental session within the 
    ...
    def start_session(self, world_model, title=None, objective=None):
        """Initializes a new experiment record."""
        if not title:
            title = f"Experiment {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    ...
    def log_event(self, entry_type, content):
        """Logs a specific event (prompt, plan, observation) to the DB."""
    ...
    def save_data(self, entry_id, device, command, payload):
        """Saves hardware data and links it to the session/entry."""
        if not self.current_exp_id:
            return None
    ...
    def end_session(self, summary=None, status="completed"):
        """Finalizes the experiment record."""
        if not self.current_exp_id:
            return
